...
for (const [browserName, browserType] of [['chromium', chromium], ['webkit', webkit]]) {
    for (const basePath of ['/', '/cfu-schedule-app/']) {
        ...
        test(`${browserName}: ${basePath} offline cold launch, cache repair and personal data`, testOptions, async t => {
            ...
            const field = name => page.locator('.profile-setup-layer label').filter({ has: page.locator('span').filter({ hasText: new RegExp(`^${name}$`) }) }).locator('select');
            const course = field('Курс');
            ...
            await field('Элективная дисциплина').selectOption('Право');
            assert.match(await field('Группа ЦК / ДРПК').innerText(), /ДРПК-101/);
            await course.selectOption('2');
            await field('Элективная дисциплина').selectOption('Цифровое моделирование');
            await field('Группа ЦК / ДРПК').selectOption('ЦК-700');
            assert.equal(await page.getByRole('button', { name: 'Показать расписание', exact: true }).isDisabled(), true);
            await field('Текущий модуль ЦК').selectOption('1');
