...
public static class CfuScheduleBackendMapper
{
    ...
    public static CfuScheduleBackendCatalog MapCatalog(CfuScheduleIndexDocument document)
    {
                 ...
                 document.Tree.OrderBy(item => item.Key, StringComparer.CurrentCultureIgnoreCase))
        {
                     ...
                     directionTree.OrderBy(item => item.Key, StringComparer.CurrentCultureIgnoreCase))
            {
                ...
                Guid directionId = CatalogStableId.Create("direction", instituteName, directionName);
                ...
                foreach ((string courseText, IReadOnlyList<string> groupCodes) in courseTree)
                {
                    ...
                    groups.AddRange(groupCodes
                        .Where(code => !string.IsNullOrWhiteSpace(code))
                        .Distinct(StringComparer.OrdinalIgnoreCase)
                        .Select(code => new StudyGroupSummary(
                            CatalogStableId.Create("group", code),
                            directionId,
                            code.Trim(),
                            courseNumber,
        ...
        return new CfuScheduleBackendCatalog(
            institutes,
            directions,
        ...
        int? subgroup = null)
    {
        ...
        IReadOnlyList<ScheduleLesson> lessons = ExpandLessons(
            index,
            schedule.Lessons.Where(lesson => subgroup is null || lesson.Subgroup == 0 || lesson.Subgroup == subgroup),
        ...
        return new ScheduleSnapshot(
            new ScheduleScope(
                ...
                CatalogStableId.Create("group", schedule.Code),
                schedule.Code.Trim(),
                subgroup is > 0
                    ? CatalogStableId.Create("subgroup", schedule.Code, subgroup.Value.ToString(CultureInfo.InvariantCulture))
                    : null),
    ...
    public static ScheduleSnapshot MapTeacher(
        CfuScheduleIndexDocument index,
        ...
        string teacherDisplayName)
    {
        ...
        ArgumentException.ThrowIfNullOrWhiteSpace(teacherDisplayName);
        IReadOnlyList<ScheduleLesson> lessons = ExpandLessons(index, sourceLessons, [], fallbackGroupCode: null);
        (DateOnly from, DateOnly to) = ResolveCoverage(index, lessons);
        return new ScheduleSnapshot(
            new ScheduleScope(ScheduleScopeKind.Teacher, teacherId, teacherDisplayName.Trim()),
            CreateVersion(lessons),
            DateTimeOffset.UtcNow,
            from,
            to,
        ...
        string? fallbackGroupCode)
    {
        IReadOnlyDictionary<int, (TimeOnly Start, TimeOnly End)> bells = index.Bells
            .Where(bell => bell.PairNumber > 0 && TryParseTime(bell.StartsAt, out _) && TryParseTime(bell.EndsAt, out _))
        ...
        foreach (CfuLessonDocument lesson in sourceLessons)
        {
            ...
            foreach (DateOnly date in ResolveLessonDates(lesson, evenMondays, oddMondays))
            {
        ...
        (TimeOnly Start, TimeOnly End) bell)
    {
        ...
        ScheduleGroupReference[] groups = string.IsNullOrWhiteSpace(normalizedGroup)
            ...
            : [new ScheduleGroupReference(
                CatalogStableId.Create("group", normalizedGroup),
                subgroup > 0 ? $"{normalizedGroup}, подгруппа {subgroup}" : normalizedGroup,
                subgroup > 0
                    ? CatalogStableId.Create("subgroup", normalizedGroup, subgroup.ToString(CultureInfo.InvariantCulture))
                    : null)];
        ...
        Guid id = CatalogStableId.Create(
            "lesson",
            normalizedGroup,
        ...
        IReadOnlyList<DateOnly> oddMondays)
    {
        ...
        return mondays.Select(monday => monday.AddDays(lesson.Day - 1)).Distinct();
    }
    ...
    private static string CreateVersion(IEnumerable<ScheduleLesson> lessons) =>
        CatalogStableId.Create("snapshot", string.Join('|', lessons.Select(lesson => lesson.Id.ToString("N"))))
            .ToString("N");
    ...
    private static bool TryParsePeriodStart(string value, out DateOnly result) =>
        TryParseDate(value.Split(['–', '—'], 2, StringSplitOptions.TrimEntries)[0], out result);
    ...
    private static TeacherSummary CreateTeacherSummary(string displayName)
    {
        string identityKey = TeacherIdentityParser.TryParse(displayName, out TeacherIdentity identity)
            ? identity.Key
            : displayName;
        return new TeacherSummary(CatalogStableId.Create("teacher", identityKey), displayName);
    }
