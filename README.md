...
- автоматическое обновление расписания ежедневно после 06:00 по Москве (UTC+03:00), независимо от часового пояса телефона: в 06:00 при открытом приложении либо при первом запуске/возврате после этого времени; при временной ошибке выполняется повтор через 15 минут, пока приложение активно.
...
Workflow `Android` собирает такой же устанавливаемый APK, публикует его как артефакт `CFU-ElJournal-Android` на 90 дней, а для тегов `v*` создаёт постоянный GitHub Release с APK и контрольной суммой SHA-256. Без отдельного production-keystore это тестовая подпись для прямой установки, а не сборка для Google Play.
...
### Временное расписание из Excel

Если официальный сайт КФУ ещё не успел обновить расписание, оператор может загрузить один `.xlsx`, папку с таблицами или ZIP-архив с таблицами КФУ. Импорт преобразует визуальный шаблон с блоками чётной/нечётной недели, подгруппами, преподавателями и аудиториями в локальный слой расписания:

```powershell
dotnet run --project src/UniversitySchedule.ScheduleImporter -- `
  --excel "C:\путь\к\расписание.zip" `
  --academic-year 2026
```

Для каждого повторного файла используется тот же ключ группы, даты недели, номера пары и подгруппы: новые строки добавляются, а совпадающие строки заменяются. Предыдущие группы сохраняются, поэтому можно передавать только изменившийся лист. Группы из Excel имеют приоритет над официальным API КФУ; для остальных групп источник не меняется. Результат сохраняется в `cfu-manual-schedule.json`, а расписания преподавателей объединяются в локальный справочник. Отчёт импорта находится в `docs/data-quality/excel-schedule-import.md`.

Это инструмент оператора на компьютере: команды выполняются из корня проекта `C:\Projects\cfu-schedule-app`. Выбора Excel-файла внутри APK/PWA пока нет. Поддерживается университетский шаблон `.xlsx`, а не произвольная таблица. Пустая ячейка или отсутствие старой пары в новом файле **не удаляет** ранее импортированное занятие.

Для полного нового расписания добавьте `--replace-excel-groups`: у групп из присланных таблиц прежние занятия заменяются целиком, включая исчезнувшие пары и записи преподавателей. Остальные группы сохраняются. Этот режим не предназначен для неполного листа с несколькими исправлениями. Общие занятия объединённых ячеек учитываются для обеих подгрупп; явные пометки «с 14.09» и «с 24.09» ограничивают даты занятий.

В v1.0.6 временно применялись восемь Excel-таблиц. По подтверждению пользователя 11 сентября 2026 года приложение в v1.0.7 возвращено на официальный API КФУ. Excel больше не перекрывает расписание; сохранённый официальный снимок служит только запасной копией. Более новый локальный кэш не заменяется этим снимком. [Текущий официальный импорт](docs/data-quality/official-schedule-import.md).

Для обновления встроенной запасной копии без повторного обхода Vuzopedia:

```powershell
dotnet run --project src/UniversitySchedule.ScheduleImporter -- --official-schedule
```

Команда заново получает официальный каталог и все его группы; при ошибке загрузки публикация данных не выполняется. Карточки преподавателей сохраняются. Снимок не фиксирует расписание навсегда: при обычном обновлении APK/PWA запрашивают выбранную группу напрямую у КФУ.

После импорта проверьте отчёт и изменения двух JSON-файлов в `src/UniversitySchedule.Mobile/Resources/Raw`, затем опубликуйте новую PWA и соберите новый APK. Импорт сам по себе не изменяет уже установленные приложения. После получения обновлённой версии нажмите **«Профиль» → «Обновить офлайн-данные»**. Эта кнопка сохраняет опубликованные данные и проверяет автономную готовность, но не импортирует Excel.

### Быстрый запуск и обновление офлайн-данных

При повторном запуске APK и PWA сначала читают выбранную группу и расписание из SQLite/IndexedDB, без ожидания сайта КФУ, полного Excel-каталога или проверки файлов PWA. Сохранённые занятия остаются видимыми во время фонового обновления и при сбое сети. Повреждённую копию можно восстановить обновлением, не удаляя личные записи.

Обновление расписания начинается после открытия интерфейса; ежедневная проверка выполняется в 06:00 МСК. Проверка полноты офлайн-версии выполняется в профиле или кнопкой **«Обновить офлайн-данные»**, не задерживая главный экран. Если данных ещё нет, их необходимо хотя бы раз загрузить с интернетом. Браузеру всё равно требуется запустить среду .NET; холодный первый запуск не становится мгновенным. iOS может приостанавливать закрытую PWA, поэтому точный запуск фоновой задачи в 06:00 МСК при закрытом приложении не гарантируется.
...
Актуальная публичная версия — **КФУ ЭлЖур v1.0.8**.

### Виджет Android

В APK v1.0.8 добавлен нативный виджет «КФУ ЭлЖур». Минимальный размер — 3×4 ячейки; виджет можно растягивать по горизонтали и вертикали. Верхние 3/7 показывают текущую пару, перемену или конец учебного дня, время, кабинет и прогресс. Нижние 4/7 показывают список пар на сегодня либо на завтра. Длинные названия прокручиваются один раз и возвращаются к началу, а красная точка обозначает заметку к занятию.

Виджет обновляется после изменения расписания или заметки и дополнительно проверяется Android примерно каждые 15 минут. Нажатие открывает приложение. Виджет использует локальный SQLite-кэш, поэтому сохранённое расписание доступно без интернета. Точная фоновая доставка обновления в 06:00 МСК зависит от ограничений Android; при следующем открытии приложение обновляет данные.

На iPhone и iPad домашние виджеты требуют отдельного WidgetKit extension-проекта и сборки на macOS/Xcode; в текущем APK реализован именно Android AppWidget.

| Платформа | Текущая версия | Установка |
| --- | --- | --- |
| Android | APK v1.0.8 | [Скачать CFU-ElJournal-Android.apk](https://github.com/l0naroff313/cfu-schedule-app/releases/download/v1.0.8/CFU-ElJournal-Android.apk) |
| iPhone и iPad | PWA v1.0.8 | [Открыть текущую PWA](https://cfu-eljournal-l0naroff313.pages.dev/) |
