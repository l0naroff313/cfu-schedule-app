...
- автоматическое обновление расписания ежедневно после 04:00 по локальному времени устройства: в 04:00 при открытом приложении либо при первом запуске/возврате после этого времени; при временной ошибке выполняется повтор через 15 минут, пока приложение активно.
...
В v1.0.6 применены восемь новых таблиц: обновлены 46 групп, ещё 23 группы сохранены. Эти таблицы имеют приоритет до сообщения пользователя об обновлении официального сайта; автоматическое обновление не переключает их обратно на сайт. [Источники и результат сверки](docs/data-quality/schedule-update-2026-09-07.md).
...
Обновление расписания начинается после открытия интерфейса; ежедневная проверка в 04:00 сохранена. Проверка полноты офлайн-версии выполняется в профиле или кнопкой **«Обновить офлайн-данные»**, не задерживая главный экран. Если данных ещё нет, их необходимо хотя бы раз загрузить с интернетом. Браузеру всё равно требуется запустить среду .NET; холодный первый запуск не становится мгновенным. iOS может приостанавливать закрытую PWA, поэтому точный запуск фоновой задачи в 04:00 при закрытом приложении не гарантируется.
...
Актуальная публичная версия — **КФУ ЭлЖур v1.0.6**.

| Платформа | Текущая версия | Установка |
| --- | --- | --- |
| Android | APK v1.0.6 | [Скачать CFU-ElJournal-Android.apk](https://github.com/l0naroff313/cfu-schedule-app/releases/download/v1.0.6/CFU-ElJournal-Android.apk) |
| iPhone и iPad | PWA v1.0.6 | [Открыть текущую PWA](https://cfu-eljournal-l0naroff313.pages.dev/) |
